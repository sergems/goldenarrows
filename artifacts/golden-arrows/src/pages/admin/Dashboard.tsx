import { useListNews, useListPlayers, useListGallery, useGetStatsSummary, useGetLeagueTable } from "@workspace/api-client-react";
import { AdminLayout } from "./AdminLayout";
import { Newspaper, Users, Image, Trophy, TrendingUp, Target } from "lucide-react";
        ...
        <div className="space-y-3">
          {news?.map(article => (
            <div key={article.id} className="flex items-center gap-4 py-3 border-b border-white/5 last:border-0">
              ...
              <span className={`text-xs px-2 py-1 rounded font-bold uppercase ${article.featured ? "bg-primary/20 text-primary border border-primary/30" : "bg-white/5 text-muted-foreground"}`}>
